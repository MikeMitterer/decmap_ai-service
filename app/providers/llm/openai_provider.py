...
class OpenAILLMProvider(LLMProvider):
    """LLM provider backed by OpenAI GPT-4o-mini."""
    ...
    async def is_spam(self, text: str, signals: list[str]) -> tuple[bool, str]:
        log = logger.bind(provider="openai", operation="is_spam")
        prompt = (
    ...
    async def translate(
        ...
        log = logger.bind(provider="openai", operation="translate", source_lang=source_lang)
        prompt = (
    ...
    async def generate_solution(
        ...
        log = logger.bind(provider="openai", operation="generate_solution")
    ...
    async def generate_tags(self, problems: list[dict]) -> list[dict]:
        log = logger.bind(provider="openai", operation="generate_tags", count=len(problems))
        summaries = [
